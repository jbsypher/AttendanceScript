...
class Application():
    ...
    def takeBoundedScreenShot(self, x1, y1, x2, y2):
        ...
        im.save(os.getcwd() + "\\Participants\\" + "zoom"+fileName+".png")
    ...
    def on_button_release(self, event):
        ...
        activeWin = str(pywinauto.findwindows.find_element(active_only=True))
        username = open("username.txt").read().strip()
        start = activeWin.find("'")
        activeWin = activeWin[start+1:]
        finish = activeWin.find("'")
        activeWin = activeWin[:finish]
        if(username==activeWin and self.i == 0):
            input()
            self.i+=1
        while True:
            print("Were you able to fit all the names in that screenshot?")
            answer = input("Yes/No: ")
            if answer[0:1].lower() == "y":
                self.exitScreenshotMode()
                return event
            elif answer[0:1].lower() == "n":
                print("Scroll down the zoom participants list and hit enter when ready to take another screenshot")
                input()
                self.createScreenCanvas()
                break
            else:
